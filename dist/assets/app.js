    ...
    function defaultWalletMethod(){
      ...
      return hasAvailableTariProvider()?'provider':globalThis.location?.origin==='https://xtm-market.johnnytsunami14.chatgpt.site'?'walletconnect':'testnet';
    }
    ...
    function renderPurchaseItem(){if(!selected)return;const image=listingImages(selected)[0]||safeImageSrc(catalogImages[selected.id]?.src),reason=purchaseBlockReason(selected);$('#purchaseItem').innerHTML=`${image?`<img class="purchase-photo" src="${image}" alt="${escapeHtml(selected.name)}">`:''}<div class="item-category">${escapeHtml(listingCategory(selected))}</div><h2>${escapeHtml(selected.name)}</h2><p>Condition: ${escapeHtml(listingCondition(selected))}</p><p>${escapeHtml(selected.description)}</p><p>Quantity: 1 · ${selected.stock} available</p><p>Seller: <span class="case-address">${escapeHtml(selected.sample?sampleSellerName(selected.id):selected.paymentAddress||'Seller not connected')}</span></p>${reason?`<div class="purchase-notice" role="status">${escapeHtml(reason)}</div>`:''}`}
